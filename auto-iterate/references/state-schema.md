...
subagents:
  - child_session_key: <session key>
    ...
    requested_agent_profile: <agent id/name or null; optional, profile-reuse workers only>
    expected_primary_model: <provider/model or null; optional, profile-reuse workers only>
    effective_model: <provider/model or null; optional, profile-reuse workers only>
    model_fallback_reason: <text or null; optional, profile-reuse workers only>
...
- If a worker was spawned from a matched named agent profile, persist `requested_agent_profile`, `expected_primary_model`, and then either `effective_model` when observed or `model_fallback_reason` explaining why the effective model differs. These fields are optional and apply only to profile-reuse workers.
